/*
	Author: Maximum
	Description: Placeables for the cop\medic sides.
	Disclaimer: Don't be a asshole and pass this off as your own or become a KAI and sell it for profit, Im releasing this for FREE… Credits: Killerty69 for a second pare of eyes when mine got to sleepy
	P.S. - Don't be a faggot like i know some of you all will be.
*/
life_barrier_active = false;
life_barrier_activeObj = ObjNull;
life_placeables = [
	[ "Cone", "RoadCone_F"],
	[ "Cone White Light", "RoadCone_L_F"],
	[ "Barrier", "EFM_mobile_barrier_B"],
	[ "Police Barrier | Long", "plp_up_WoodBarrierLongPolice"],
	[ "Police Barrier | Short - Lights", "plp_up_WoodBarrierLongPoliceLightsOn"],
	[ "Police Barrier | Long", "plp_up_BarrierFenceMetal"],
	[ "Conc Barrier", "EFM_concrete_barrier_new"],
	[ "Site Fence W/Tarp", "plp_up_SiteFenceMetalTarpBlackA"],
	[ "Site Fence (Bare)", "plp_up_SiteFenceMetalBare"],
	[ "Sandbag Wall", "Land_BagFence_Short_F"],
	[ "Single Lamp", "Land_PortableLight_single_F"]
];
life_bar_limit = 2500;
life_bar_placey = [];
