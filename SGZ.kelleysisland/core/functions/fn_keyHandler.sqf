...
switch (_code) do {
    ...
    case 34: {
        if (_shift) then {_handled = true;};
        if (_shift && !(playerSide isEqualTo independent) && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
	...
	if(_shift) then {
		switch (player getVariable["Earplugs",0]) do {
			case 0: {hintSilent "Ear Plugs 90%"; 1 fadeSound 0.1; player setVariable ["Earplugs", 10]; life_earplugs = true; [] call life_fnc_hudUpdate; };
			case 10: {hintSilent "Ear Plugs 60%"; 1 fadeSound 0.4; player setVariable ["Earplugs", 40]; life_earplugs = true; [] call life_fnc_hudUpdate; };
			case 40: {hintSilent "Ear Plugs 30%"; 1 fadeSound 0.7; player setVariable ["Earplugs", 70]; life_earplugs = true; [] call life_fnc_hudUpdate; };
			case 70: {hintSilent "Ear Plugs Removed"; 1 fadeSound 1; player setVariable ["Earplugs", 0]; life_earplugs = false; [] call life_fnc_hudUpdate; };
		    };
	   };
   };
