...
Dvid_TFEnabled = true;
Dvid_onTsServer = "ScottishGAMERZ Altis Life :: Welcome" == (call TFAR_fnc_getTeamSpeakServerName); ///////////////////////Edit This line (the channel at the top of your TS (where you right click and "Edit Virtual Server"), yes, get the name of that goes here)
...
[] spawn {
	...
	while {true} do {
				...
				if (!(isNil "life_adminlevel")) then {
					...
					_onTsServer = "ScottishGAMERZ Altis Life :: Welcome" == (call TFAR_fnc_getTeamSpeakServerName); //////////////////////Edit too pls, or dont but it wont work if you dont
					if (!(_onTsServer)) then {
					if (!(_isadmin)) then {
